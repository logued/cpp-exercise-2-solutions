...
void increment(int& intRef)	// intRef  is  a reference to an integer
{
		intRef++;   // increments the value referred to by this reference
                    // (intRef refers to the variable 'num'
}
...
void swapByReference(int& a, int& b )   // references to ints
{
	...
	a = b;          // b is areference to  y (below), so b accesses y
	b = temp;
}
...
// be swapped and the values in x and y in the calling function  will not change
